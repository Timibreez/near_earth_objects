...
collection of objects that is used by
the `query` method to generate a stream of
`CloseApproach` objects that match
all of the desired criteria. The arguments to
`create_filters` are provided by
the main module and originate from the
user's command-line options.

This function can be thought to return a
collection of instances of subclasses
of `AttributeFilter` - a 1-argument
...
The `limit` function simply limits the
maximum number of values produced by an iterator.
...
class AttributeFilter:
    ...
    essentially functions as a callable
    ...
    Concrete subclasses can override the
...
def create_filters(
    ...
    selects close approaches that occurred
    ...
    selects close approaches whose nominal
    approach distance is at least that
    ...
    because the main module directly passes
    this result to that method. For now,
    ...
    :param distance_min: A minimum nominal approach
    distance for a matching `CloseApproach`.
    :param distance_max: A maximum nominal approach
    distance for a matching `CloseApproach`.
    :param velocity_min: A minimum relative approach
    velocity for a matching `CloseApproach`.
    :param velocity_max: A maximum relative approach
    velocity for a matching `CloseApproach`.
    :param diameter_min: A minimum diameter of the
    NEO of a matching `CloseApproach`.
    :param diameter_max: A maximum diameter of the
    NEO of a matching `CloseApproach`.
